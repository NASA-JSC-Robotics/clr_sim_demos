...
public:
  RunDemoNode(rclcpp::NodeOptions node_options)
    ...
    apply_planning_scene_client =
        this->create_client<moveit_msgs::srv::ApplyPlanningScene>(
            "/apply_planning_scene");
    if (hw) {
      bench_publisher =
          this->create_publisher<std_msgs::msg::Float64>("bench_position", 10);
    }
  ...
  void run_demo() {
    ...
    moveit_viz->trigger();
    moveit_viz->prompt("Press next to start demo");
    if (!this->init()) {
    ...
    if (!this->open_bench_seat()) {
      RCLCPP_ERROR(LOGGER, "Failed to reach bench seat pose 1. Exiting.");
      return;
    }
    if (!this->back_out()) {
    ...
    if (!this->traverse_left()) {
      RCLCPP_ERROR(LOGGER,
                   "Failed to traverse to CTB dropoff location. Exiting.");
      return;
    }
  ...
  bool open_bench_seat() {
    ...
    auto hinge_pose =
        dex_ivr::transform_stamped_to_pose_stamped(hinge_in_world);
    // get grasp frame
    ...
    auto grasp_pose =
        dex_ivr::transform_stamped_to_pose_stamped(grasp_in_world);
    auto arc_plan = std::vector<Waypoint>();
    for (int i = 1; i <= 8; i++) {
      // returns pose in rotation frame
      geometry_msgs::msg::PoseStamped local_pose = dex_ivr::rotate_about_frame(
          "bench_lid", hinge_pose, grasp_pose, -i * M_PI / 15.6, "y", true);
    ...
    this->plan_and_execute(arc_plan);
  ...
  bool approach_ctb_handle() {
    ...
    do {
      ...
      response = this->request_response<
          ...
          std::shared_ptr<dex_ivr_interfaces::srv::BlobCentroid::Request>,
          std::shared_ptr<dex_ivr_interfaces::srv::BlobCentroid::Response>>(
  ...
  bool pre_drop_ctb() {
    ...
    Waypoint approach_wp_1 =
        Waypoint(eef.transform.translation.x, eef.transform.translation.y,
                 eef.transform.translation.z, 0.725, 0.688, 0.032, -0.004,
                 "ur_manipulator", true);
  ...
  bool plan_and_execute(const std::vector<Waypoint> &waypoints,
                        bool setup = true) {
    ...
    for (const auto &waypoint : waypoints) {
      ...
      // presets.
      if (waypoint.planning_group != group || waypoint.is_relative ||
          waypoint.use_jconfig || waypoint.is_preset) {
        RCLCPP_WARN(
            LOGGER,
            "Cannot plan a Cartesian trajectory for provided waypoints.");
        return false;
      } else if (!waypoint.plan_cartesian) {
        RCLCPP_WARN(
            LOGGER,
            "Cannot plan non-Cartesian trajectory for multiple waypoints.");
        return false;
      }
...
private:
  ...
  bool set_collisions(bool allowed, std::string link1, std::string link2 = "") {
    ...
    auto get_planning_scene =
        std::make_shared<moveit_msgs::srv::GetPlanningScene::Request>();
    auto response = this->request_response<
        rclcpp::Client<moveit_msgs::srv::GetPlanningScene>::SharedPtr,
    ...
    // Modify ACM
    auto acm = collision_detection::AllowedCollisionMatrix(
    ...
    auto apply_response = this->request_response<
        rclcpp::Client<moveit_msgs::srv::ApplyPlanningScene>::SharedPtr,
    ...
    if (!apply_response->success) {
      if (link2 == "") {
        RCLCPP_ERROR(LOGGER, "Failed to disable collisions with %s.",
                     link1.c_str());
      } else {
        RCLCPP_ERROR(LOGGER, "Failed to disable collisions between %s and %s.",
  ...
  bool get_global_transform(const std::string &frame_id,
    ...
    try {
      t = this->tf_buffer->lookupTransform("world", frame_id,
                                           tf2::TimePointZero,
                                           std::chrono::nanoseconds(5000));
    } catch (const tf2::TransformException &ex) {
      RCLCPP_INFO(this->get_logger(),
                  "Could not get transform from %s to world: %s",
  ...
  bool plan_cartesian(const std::vector<geometry_msgs::msg::Pose> &poses,
                      moveit_msgs::msg::RobotTrajectory &trajectory) {
    ...
    double trajectory_percent = move_group->computeCartesianPath(
        poses, eef_step, jump_threshold, trajectory);
    ...
    robot_trajectory::RobotTrajectory rt(
        move_group->getCurrentState()->getRobotModel(), move_group->getName());
  ...
  void parameter_setup() {
    ...
    prompt_desc.description =
        "If true, the bench seat position will be published as a substitute "
  ...
  void load_waypoints_from_yaml(std::string demo_config) {
    ...
    for (YAML::const_iterator it = wp_yaml["waypoints"].begin();
         it != wp_yaml["waypoints"].end(); ++it) {
      if (it->second["config"]) {
        Waypoint loaded_waypoint =
            Waypoint(it->second["config"].as<std::vector<double>>(),
                     it->second["planning_group"].as<std::string>(),
        ...
        Waypoint loaded_waypoint =
            Waypoint(it->second["pose"]["x"].as<double>(),
                     it->second["pose"]["y"].as<double>(),
                     it->second["pose"]["z"].as<double>(),
                     it->second["pose"]["qx"].as<double>(),
                     it->second["pose"]["qy"].as<double>(),
                     it->second["pose"]["qz"].as<double>(),
                     it->second["pose"]["qw"].as<double>(),
                     it->second["planning_group"].as<std::string>(),
                     it->second["cartesian"].as<bool>());
        ...
        Waypoint loaded_waypoint =
            Waypoint(it->second["preset_name"].as<std::string>(),
                     it->second["planning_group"].as<std::string>());
        if (it->second["planner"])
          loaded_waypoint.planner = it->second["planner"].as<std::string>();
        wp_map.insert({it->first.as<std::string>(), loaded_waypoint});
